...
def load_checkpoint():
    """
    Load existing labels from partitioned checkpoints - ONLY reads IDs to save RAM!

    Reads partition files one at a time, extracts only IDs, never loads full data.
    """
    # Check for partition files (new system)
    ...
    if partition_files:
        ...
        # Read each partition ONE AT A TIME, extract only IDs, then discard
        all_data = []
        for pf in partition_files:
            df_part = pd.read_parquet(pf, columns=['dataset', 'geometry_id'])
            all_data.append(df_part)

        # Combine just the IDs
        df_ids_only = pd.concat(all_data, ignore_index=True)
        print(f"  ✓ Loaded {len(df_ids_only):,} existing IDs from partitions")
        return df_ids_only

    # Fallback: check old checkpoint file (for backward compatibility)
    checkpoint_file = CHECKPOINT_DIR / "labels_checkpoint.parquet"
    if checkpoint_file.exists():
        try:
            df = pd.read_parquet(checkpoint_file, columns=['dataset', 'geometry_id'])
            print(f"  ✓ Loaded {len(df):,} existing IDs from checkpoint")
            return df
        except Exception as e:
...
def save_checkpoint(df_new_labels, dataset_name=None):
    """
    Save checkpoint using PARTITIONED files - NEVER loads full checkpoint into RAM!

    Each batch writes to its own partition file. Files are merged only at the very end.
    """
    import time

    # Create unique partition file for this batch
    timestamp = int(time.time() * 1000)  # millisecond precision
    partition_file = CHECKPOINT_DIR / f"checkpoint_part_{timestamp}.parquet"

    # Write just this batch to its own file (never touches existing data)
    df_new_labels.to_parquet(partition_file, index=False)

    if dataset_name:
        # Count total by counting partition files (fast, no data loading)
...
def process_single_row(args):
    """
    Wrapper for parallel processing of a single geometry.

    Args:
        args: Tuple of (geom_id, n_moduli, dataset_name, seed)

    Returns:
        Dict with label results
    """
    geom_id, n_moduli, dataset_name, seed = args
    label = generate_label_for_geometry(geom_id, n_moduli, n_samples=3, seed=seed)
    label['dataset'] = dataset_name
    return label


def main():
    """Generate stability labels for all datasets with IMPROVED ALGORITHMS + PARALLEL PROCESSING."""
    print("=" * 70)
    ...
    if existing_df is not None:
        # MEMORY FIX: Don't load full data into RAM!
        # Only extract the IDs to know what's been processed
        ...
        print(f"  ✓ Processed IDs loaded (checkpoint data stays on disk to save RAM)")
        print()

        # Don't keep existing_df in memory - let it be garbage collected
        del existing_df
        import gc
        gc.collect()

    # Start with empty list - we'll only keep new labels in RAM until next checkpoint
    all_labels = []
    ...
    for filename, id_col, moduli_col in datasets:
        ...
        N_LIMIT = None  # Full dataset: ALL 270,659 geometries (~1 hour total)
        ...
        checkpoint_interval = 100  # Checkpoint every 100 samples
        all_labels = []
        ...
        for chunk_idx in range(num_chunks):
            ...
            with Pool(processes=n_processes) as pool:
                chunk_results = list(tqdm(
                    pool.imap(process_single_row, chunk_tasks),
            ...
            if all_labels:
                ...
                # CRITICAL FIX: Clear all_labels to free memory!
                # The checkpoint is already saved to disk, so we don't need to keep it in RAM
                all_labels = []
        ...
        if all_labels:  # Only save if there are new labels
            df_checkpoint = pd.DataFrame(all_labels)
            save_checkpoint(df_checkpoint, dataset_name)
            all_labels = []  # Clear after saving

    # Merge all partition files into final output
    print("\nFinalizing output...")
    partition_files = sorted(CHECKPOINT_DIR.glob("checkpoint_part_*.parquet"))
    ...
    if partition_files:
        ...
        # Read and merge partitions in small chunks to save memory
        all_chunks = []
        for i, pf in enumerate(partition_files, 1):
            df_chunk = pd.read_parquet(pf)
            all_chunks.append(df_chunk)

            # Periodically merge and clear to save memory
            if len(all_chunks) >= 50:  # Merge every 50 partitions
